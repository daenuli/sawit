<?php
include "config/koneksi.php"; $table = "penimbanganPelabuhan"; $kd = "noSlip"; 
include "config/fungsi_form.php";
include "config/inc.library.php";
	if(!isset($_GET['page'])){$page = 'data';}else{$page = $_GET['page'];} // $page

		if(isset($_GET['noKendaraan'])){
			$qryKendaraan = mysql_query("SELECT noKendaraan, nmRelasi FROM tb_kendaraan WHERE noKendaraan = '$_GET[noKendaraan]'");
			$rowKendaraan = mysql_fetch_array($qryKendaraan);
			$jumKendaraan = mysql_num_rows($qryKendaraan);
			if($jumKendaraan !== 0){
			
				$qry = mysql_query("SELECT * FROM $table WHERE noKendaraan = '$_GET[noKendaraan]' AND complate = 'n'");
				$jumQ = mysql_num_rows($qry);
				if($jumQ == 0){$page = 'add';}else{$page = "edit";}
				
				
			}else{echo '<div class="alert alert-danger alert-white rounded">
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
								<div class="icon"><i class="fa fa-times-circle"></i></div>
								<strong>Error!</strong> Kendaraan is not Found, try again.
							 </div>';}	
	...
	if(isset($_POST['btnSubmit'])){ $btnSubmit = $_POST['btnSubmit'];
		...
		if($btnSubmit == 'edit'){
				...
				if($qry){ ?>
							<script>
									window.open('laporan/strukPelabuhan.php?noSlip=<?=$_POST[$kd]?>');	
							</script>	 <?php
...
<tbody class="no-border-x no-border-y">
	...
	<tr>
	  ...
	  <td width="1%"><b>:</b></td>
	  <td width="79%"><input name="noSlip" id="noSlip" value="<?=buatKodeFaktur("penimbanganPelabuhan","P");?>" size="11" maxlength="11" readonly="readonly"/>
      </td></tr>
	<tr>
	...
	<tr>
	  ...
	  <td><b>:</b></td>
	  <td><b>
	    No. Slip Tambang :<input name="noSlipTambang" id="noSlipTambang" value="" size="11" maxlength="11" required="required"/>
        Barat Tambang :<input name="beratTambang" id="beratTambang" value="" size="11" maxlength="11" required="required"/>Kg.
      </b></td>
    </tr>     
	<tr><td>&nbsp;</td>
	  <td>&nbsp;</td>
	...
	<tr>
	  ...
	  <td><b>:</b></td>
	  <td><b>
	    <b>Bruto </b><input name="bruto" class="angkaC" id="bruto" size="14" maxlength="20" readonly="readonly" value="" required="required" />
<button name="getTimbangan" onclick="getBruto()" id="getTimbangan" class="btn btn-warning btn-sm" type="button" style="cursor:pointer;">Get</button>
      </b></td>
    </tr>
	<tr>
	  <td>&nbsp;</td>
...
<tbody class="no-border-x no-border-y">
	...
	<tr>
	  ...
	  <td><b>:</b></td>
	  <td><b>
	    No. Slip Tambang :<input name="noSlipTambang" id="noSlipTambang" value="<?=$rowEdit['noSlipTambang']?>" size="11" maxlength="11" readonly="readonly"/>
        Barat Tambang :<input name="beratTambang" id="beratTambang" value="<?=$rowEdit['beratTambang']?>" size="11" maxlength="11" readonly="readonly"/>Kg.
      </b></td>
    </tr>        
	<tr><td>&nbsp;</td>
	  <td>&nbsp;</td>
	...
	<tr>
	  ...
	  <td><b>:</b></td>
	  <td><b>
	    <b>Bruto </b><input name="bruto" class="angkaC" id="bruto" value="<?=$rowEdit['bruto']?>" size="14" maxlength="20" readonly="readonly" required="required" />
		<b>Tara </b><input name="tara" class="angkaC" id="tara" value="" size="14" maxlength="20" readonly="readonly" required="required" />
        
<button onclick="getTara()" name="getTimbangan" id="getTimbangan" class="btn btn-warning btn-sm" type="button">Get</button>
...
function getTara(){
 			  $.get('indikator.php', function(data){
				  ...
				  $('#netto').val($('#bruto').val() - parseInt(data)); 
				  $('#getTimbangan').hide();
				  };
			  });
};
