...
function Header()
{
	...
	$this->SetY(10);
	$this->Cell(0,10,'laporan PELABUHAN per HARI '.tgl_indo($_GET['awal']).' - '.tgl_indo($_GET['akhir']),0,0,'R');
	// Line break
	$this->Ln(12);
...
function ImprovedTable()
{
	...
	$queryHead  = mysql_query("select DISTINCT SUBSTR(tanggalKeluar,1,10) as tglKeluar from penimbanganPelabuhan WHERE SUBSTR(tanggalKeluar,1,10) BETWEEN '$_GET[awal]' and '$_GET[akhir]' order by tanggalKeluar asc");
	$gb = 0; $gt = 0; $gn = 0;
	while($rH = mysql_fetch_array($queryHead)){
	$this->Cell(0,10,tgl_indo($rH['tglKeluar']),0,0,'L');
	$this->Ln();
	// Header
	...
	// Data
	$nomor = 0;
	$qry = mysql_query("select * from penimbanganPelabuhan where SUBSTR(tanggalKeluar,1,10) = '$rH[tglKeluar]' and SUBSTR(tanggalKeluar,1,10) BETWEEN '$_GET[awal]' and '$_GET[akhir]' order by tanggalKeluar asc");
	$tb = 0; $tt = 0; $tn = 0;
