...
function Header()
{
	...
	$this->SetY(10);
	$this->Cell(0,10,'laporan Lainnya per RELASI '.tgl_indo($_GET['awal']).' - '.tgl_indo($_GET['akhir']),0,0,'R');
	// Line break
	$this->Ln(12);
...
function ImprovedTable()
{
	...
	$queryHead  = mysql_query("select DISTINCT nmRelasi from penimbanganLainya_d WHERE nmRelasi = '$_GET[relasi]' and SUBSTR(tanggalKeluar,1,10) BETWEEN '$_GET[awal]' and '$_GET[akhir]' order by tanggalKeluar asc");
	$gb = 0; $gt = 0; $gn = 0;
	while($rH = mysql_fetch_array($queryHead)){
	$this->Cell(0,10,$rH['nmRelasi'],0,0,'L');
	$this->Ln();
	// Header
	...
	// Data
	$nomor = 0;
	$qry = mysql_query("select * from penimbanganLainya_d where nmRelasi = '$rH[nmRelasi]' and SUBSTR(tanggalKeluar,1,10) BETWEEN '$_GET[awal]' and '$_GET[akhir]' order by tanggalKeluar asc");
	$tb = 0; $tt = 0; $tn = 0;
	while($r = mysql_fetch_array($qry)){
		//$this->Cell($w[0],6,$nomor.'0','LR');
		$this->Cell($w[0],6,$r['nmSlipLainya'],'LR',0,'C');
		...
		$this->Cell($w[5],6,$r['nmKendaraan'],'LR',0,'L');
	...
	$this->Ln(5);
		...
		$this->Cell($w[6],6,'Total Bruto: ','',0,'L');
	...
	$this->Ln(5);
		...
		$this->Cell($w[6],6,'Total Tara: ','',0,'L');
	...
	$this->Ln(5);
		...
		$this->Cell($w[6],6,'Total Netto: ','',0,'L');
