...
function ImprovedTable()
{
	...
	$queryHead  = mysql_query("select DISTINCT SUBSTR(tanggalKeluar,1,10) as tglKeluar from penimbanganTambang WHERE SUBSTR(tanggalKeluar,1,10) BETWEEN '$_GET[awal]' and '$_GET[akhir]' order by tanggalKeluar asc");
	$gb = 0; $gt = 0; $gn = 0;
	...
	// Data
	$nomor = 0;
	$qry = mysql_query("select * from penimbanganTambang where SUBSTR(tanggalKeluar,1,10) = '$rH[tglKeluar]' and SUBSTR(tanggalKeluar,1,10) BETWEEN '$_GET[awal]' and '$_GET[akhir]' order by tanggalKeluar asc");
	$tb = 0; $tt = 0; $tn = 0;
