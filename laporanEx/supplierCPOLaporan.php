...
function Header()
{
	...
	$this->SetY(10);
	$this->Cell(0,10,'laporan CPO per RELASI '.tgl_indo($_GET['awal']).' - '.tgl_indo($_GET['akhir']),0,0,'R');
	// Line break
	$this->Ln(12);
...
function ImprovedTable()
{
	...
	$queryHead  = mysql_query("select DISTINCT nmRelasi from penimbanganCPO_d WHERE nmRelasi = '$_GET[relasi]' and SUBSTR(tanggalKeluar,1,10) BETWEEN '$_GET[awal]' and '$_GET[akhir]' order by tanggalKeluar asc");
	$gb = 0; $gt = 0; $gn = 0;
	...
	// Data
	$nomor = 0;
	$qry = mysql_query("select * from penimbanganCPO_d where nmRelasi = '$rH[nmRelasi]' and SUBSTR(tanggalKeluar,1,10) BETWEEN '$_GET[awal]' and '$_GET[akhir]' order by tanggalKeluar asc");
	$tb = 0; $tt = 0; $tn = 0;
	while($r = mysql_fetch_array($qry)){
		//$this->Cell($w[0],6,$nomor.'0','LR');
		$this->Cell($w[0],6,$r['nmSlipCPO'],'LR',0,'C');
