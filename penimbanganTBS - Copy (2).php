...
include "config/inc.library.php";
	...
	if(isset($_POST['btnSubmit'])){ $btnSubmit = $_POST['btnSubmit'];
		if($btnSubmit == 'add'){
			...
			$brutoAsli = ($_POST['bruto'] / (100 - $rowRelasi['potongan'])) * 100;
		...
		if($btnSubmit == 'edit'){
			$potPers = $_POST['potWajib'] + $_POST['potSampah'] + $_POST['potTangkai'] + $_POST['potPasir'] + $_POST['potAir'] + $_POST['potMutu'];
				...
				if($qry){ ?>
							<script>
									window.open('laporanEx/slipTBS.php?noSlip=<?=$_POST[$kd]?>');	
							</script>	 
						<?php echo "<meta http-equiv='refresh' content='0; url=?page=data'>";}
		} // ===========================================
	}
