...
# Fungsi untuk membuat kode automatis
function buatKodeFaktur($tabel, $inisial){
	$struktur	= mysql_query("SELECT * FROM $tabel");
	$field		= mysql_field_name($struktur,0);
	$panjang	= mysql_field_len($struktur,0);

	$tglBuln = date('Ymd');
 	$qry	= mysql_query("SELECT MAX(".$field.") FROM ".$tabel." WHERE SUBSTR(".$field.",1,8) = '$tglBuln'");
 	$row	= mysql_fetch_array($qry); 
 	if ($row[0]=="") {
 		$angka=0;
	}
 	else {
 		$angka		= substr($row[0], strlen($inisial)+8);
 	}
	
 	$angka++;
 	$angka	=strval($angka); 
 	$tmp	="";
 	for($i=1; $i<=($panjang-strlen($inisial)-strlen($angka)-8); $i++) {
		$tmp=$tmp."0";	
	}
 	//return $inisial.$tmp.$angka;
	return $inisial.$tglBuln.$tmp.$angka;
}
